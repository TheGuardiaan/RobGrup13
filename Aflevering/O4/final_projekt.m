...
% drivePath(punkt_A, controller, robotPub, map, nodes); %(Goal,controller) 
...
%     end
   ...
   if (dist < minDist)
       ...
       if(dist >= minDist)  % if obstacle moved
           obstacleDetected = 0;
           return;
       end       
       ...
       while(dist < minDist)
            ...
            pause(1);
            sendVelmsgRob(0, 0, robotPub); % stop
